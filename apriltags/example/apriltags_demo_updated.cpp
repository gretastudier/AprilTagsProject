...
public:
  ...
  void print_detection(AprilTags::TagDetection& detection) {
    ...
    cout << "  distance=" << translation.norm()
         ...
         << ", pitch=" << pitch
         << ", roll=" << roll
	<<"\n inner_counter:" << count_in
         << endl;
    

    if(count_in == detection_size) {
      count++;
	}
    count_in++;
  ...
  void datafile() {
    ...
    myfile << "Count,Time,Tag ID,Distance,X,Y,Z,Pitch,Roll,Yaw\n"
                << endl;
    count = 1;
   }
  ...
  void processImage(cv::Mat& image, cv::Mat& image_gray) {
    ...
    detection_size = detections.size(); //Detection size global variable
